...
tape("rgbaToI420(rgbaFrame, i420Frame)", (t) => {
  t.test("it works", (t) => {
    ...
    rgbaToI420(rgbaFrame, i420Frame);
    t.ok(
      everyYuv(i420Frame, [173, 143, 31], 1),
...
function everyYuv(i420Frame, yuv, tolerance = 0) {
  for (let i = 0; i < i420Frame.byteLength; i++) {
    if (i < i420Frame.sizeOfLuminancePlane) {
      const y = i420Frame.data[i];
      if (Math.abs(y - yuv[0]) > tolerance) {
      ...
      if (Math.abs(u - yuv[1]) > tolerance) {
        console.log("u", u, yuv[1]);
        return false;
      }
    } else {
      const v = i420Frame.data[i];
      if (Math.abs(v - yuv[2]) > tolerance) {
