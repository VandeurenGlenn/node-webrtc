...
function labelScenario(name) {
  return name
    .replace(/_ms$/, "")
    .replace(/_messages_per_second$/, "")
    .replace(/_mib_per_second$/, "")
    .split("_")
...
function deltaDisplay(comparison) {
  ...
  const slower = comparison.lowerIsBetter === false
    ? comparison.deltaPercent < 0
    : comparison.deltaPercent > 0;
  const lowerIsBetter = comparison.lowerIsBetter !== false;
  return {
    icon: slower
      ? (lowerIsBetter ? "↑" : "↓")
      : (lowerIsBetter ? "↓" : "↑"),
...
function scenarioUnit(scenario) {
  return scenario.unit || "ms";
}

function formatScenarioValue(value, scenario) {
  return `${value.toFixed(3)} ${scenarioUnit(scenario)}`;
}
...
function renderTimeline(result, scenarioName) {
  ...
  const dots = coordinates.map(({ x, y, value, point }) =>
    `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4"><title>${escapeHtml(point.meta.commit.slice(0, 8))}: ${formatScenarioValue(value, result.scenarios[scenarioName])}</title></circle>`).join("");
...
const markdown = [
  ...
  "",
  "Latency: lower is better. Throughput: higher is better. Baselines come from the latest successful `develop` run on the same platform and architecture.",
  "",
];
...
for (const result of results) {
  ...
  for (const [name, scenario] of Object.entries(result.scenarios)) {
    ...
    markdown.push(
      `| ${labelScenario(name)} | ${baseline === "—" ? baseline : formatScenarioValue(baseline, scenario)} | ${formatScenarioValue(scenario.mean, scenario)} | ${delta.icon} ${delta.text} |`,
    );
  }
  markdown.push("");
}
...
const cards = results.map((result) => {
  const rows = Object.entries(result.scenarios).map(([name, scenario]) => {
    ...
    return `<section class="scenario">
      ...
      <div class="bar-row"><span>Current</span><div class="track"><i class="current" style="width:${currentWidth}%"></i></div><strong>${formatScenarioValue(scenario.mean, scenario)}</strong></div>
      ${baseline == null ? "" : `<div class="bar-row"><span>Baseline</span><div class="track"><i class="baseline" style="width:${baselineWidth}%"></i></div><strong>${formatScenarioValue(baseline, scenario)}</strong></div>`}
...
const scenarioFastest = Object.fromEntries(implementationScenarioNames.map((name) => [
  name,
  projectResult?.scenarios[name]?.lowerIsBetter === false
    ? Math.max(...comparableImplementations.map((result) => result.scenarios[name].median))
...
const overallRanking = comparableImplementations.map((result) => {
  const ratios = implementationScenarioNames.map((name) =>
    result.scenarios[name].lowerIsBetter === false
      ? scenarioFastest[name] / result.scenarios[name].median
...
const implementationCharts = implementationScenarioNames.map((name) => {
  const lowerIsBetter = projectResult?.scenarios[name]?.lowerIsBetter !== false;
  ...
  const scenario = projectResult?.scenarios[name] || { unit: "ms" };
  const rows = entries.map((entry, index) => {
    const slower = fastest === 0
      ? 0
      : lowerIsBetter
        ? ((entry.value - fastest) / fastest) * 100
        : ((fastest - entry.value) / fastest) * 100;
    const status = index === 0
      ? (lowerIsBetter ? "fastest" : "highest")
      : lowerIsBetter
...
</style></head><body><h1>node-webrtc benchmarks</h1><p class="intro">Lower latency and higher throughput are better. Current commit is compared with the latest successful develop build on identical runner architecture.</p><main class="grid">${cards}</main>${implementationSection}<footer>Generated ${escapeHtml(new Date().toISOString())}</footer></body></html>`;
